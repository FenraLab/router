...
> {
  ...
  public handlers: Map<
    string,
    Middleware.TFunction<[TRequest, TResponse], TError>[]
  > = new Map();
...
> extends Endpoint<TRequest, TResponse, TError> {
  ...
  route<TFurther extends Endpoint<TRequest, TResponse, TError>>(
...
> {
  ...
  constructor(copyConstruct?: RouterVisitor<TRequest, TResponse, TEndpoint>) {
    if (copyConstruct) {
